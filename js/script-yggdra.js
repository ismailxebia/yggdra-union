...
function updateCharacterPosition() {
    document.querySelectorAll('.gridCell').forEach(cell => {
        ...
        cell.removeEventListener('click', handleInitialPositionClick);

        if (row === characterPosition.x && col === characterPosition.y) {
            cell.classList.add('character');
            if (turnPhase === 'allies' && !isActionMenuActive && !isCharacterSelected) {
        ...
        if (initialPosition && row === initialPosition.x && col === initialPosition.y) {
            ...
            if (isCharacterSelected) {
                cell.addEventListener('click', handleInitialPositionClick);
            }
        }
...
function handleInitialPositionClick() {
    performAction('back');
}
...
function performAction(action) {
    if (action === 'move') {
        ...
        resetCharacterState();
        hideActionMenu(); // Pastikan untuk menyembunyikan action menu setelah back
    }
}
...
function endTurn() {
    if (turnPhase === 'allies') {
        ...
        resetCharacterState();  // Reset state karakter di awal giliran allies
    }
}
